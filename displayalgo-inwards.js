// Our Javascript will go here.
        ...
        var geometry = new THREE.PlaneGeometry( 1200, 2000, 32 );
        var material = new THREE.MeshBasicMaterial( {color: 0x20507b, side: THREE.DoubleSide} );
...
/////////////////////////////////////////////////////////////////////////////////////////
        ...
        function modifyStack(stackId, lamina_top, lamina_medium, lamina_low,issearch){
            paintTopLamina(stackId, lamina_top);
            paintMediumLamina(stackId, lamina_top, lamina_medium);
            paintLowLamina(stackId, lamina_top, lamina_medium, lamina_low,issearch);
        }
        ...
        function paintTopLamina(stackId, lamina_top) {
            for (let i = 0; i < lamina_top; i++) {
                ...
                if (stackId>=15)
                    newGroup[stackId].getObjectByName(i).material.color.setHex( 0xffb3b3);
                else
        ...
        function paintMediumLamina(stackId, lamina_top, lamina_medium) {
            for (let i = lamina_top; i < lamina_medium + lamina_top; i++) {
                newGroup[stackId].getObjectByName(i).visible = true;
                if (stackId>=15)
                    newGroup[stackId].getObjectByName(i).material.color.setHex( 0xcffcf7);
                else
                    newGroup[stackId].getObjectByName(i).material.color.setHex( 0x41f4df );
            }
        }

        function paintLowLamina(stackId, lamina_top, lamina_medium, lamina_low,issearch) {
            for (let i = lamina_top + lamina_medium; i < lamina_top + lamina_medium + lamina_low; i++) {
                newGroup[stackId].getObjectByName(i).visible = true;
                if ((stackId>14) || (stackId==14 && i>181) )
                {
        ...
        function cloneStack(number, cubesPerStackArray,issearch){
            ...
            loader.load( 'https://s3.amazonaws.com/uploadsfreecreatives/fonts/helvetiker_regular.typeface.json', function ( font ) {
                ...
                for (var i=0;i<cubesPerStackArray.length;i++)
                {
                    ...
                    modifyStack(cubesPerStackArray[i][0]-1, cubesPerStackArray[i][1], cubesPerStackArray[i][2], cubesPerStackArray[i][3],issearch);
                }
